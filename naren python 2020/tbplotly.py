...
fig.add_trace(
    go.Scatter(x=list(df.index),
               y=list(df.Relative_Humidity),
               name="Relative Humidity",
               line=dict(color="#F06A6A")))

fig.add_trace(go.Scatter(x=list(df.index),y=[df.Relative_Humidity.mean()] * len(df.index),name="Relative Humidity Average",visible=False,line=dict(color="#F06A6A", dash="dash")))
...
low_annotations = [dict(x="2015-05-01",
                        y=df.Relative_Humidity.mean(),
                        xref="x", yref="y",
                        text="Relative_Humidity Average:<br> %.2f" % df.Relative_Humidity.mean(),
                        ax=-40, ay=40),
                   dict(x=df.Rainfall.idxmin(),
                        y=df.Relative_Humidity.min(),
                        xref="x", yref="y",
                        text="Relative_Humidity Min:<br> %.2f" % df.Relative_Humidity.min(),
                        ax=0, ay=40)]
